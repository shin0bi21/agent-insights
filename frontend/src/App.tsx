...
export default function App() {
  ...
  return <>
    ...
    <main id="main">
      ...
      </div></> : view === 'history' ? <section className="history-view" aria-labelledby="history-title"><div className="page-heading"><div><p className="kicker">RUN ARCHIVE</p><h2 id="history-title">Run history</h2><p>Review every run before the latest one, including interrupted attempts.</p></div><button className="quiet" onClick={() => void loadRuns()} type="button">Refresh history</button></div><section className="panel"><div className="runs history-runs" aria-live="polite">{historyRuns.length ? historyRuns.map(run => <RunCard key={run.id} run={run} now={now} retryDisabled={busy || runInProgress} onRetry={item => void retry(item)}/>) : <p className="empty">No earlier runs yet.</p>}</div></section></section> : <section className="settings-view" aria-labelledby="settings-title"><div className="page-heading"><div><p className="kicker">PREFERENCES</p><h2 id="settings-title">Settings</h2><p>Choose how Repo Automation Score looks on this machine.</p></div></div><section className="panel settings-panel"><h3>Appearance</h3><div className="theme-options" role="radiogroup" aria-label="Color theme">{(['light', 'dark'] as Theme[]).map(option => <button key={option} className="theme-option" role="radio" aria-checked={theme === option} onClick={() => setTheme(option)} type="button"><span className={`theme-preview ${option}`} aria-hidden="true"><i/><i/><i/></span><strong>{option === 'light' ? 'Light' : 'Dark'}</strong><small>{option === 'light' ? 'White with purple accents' : 'Deep charcoal with purple accents'}</small></button>)}</div></section></section>}
    </main>
  </>;
}
