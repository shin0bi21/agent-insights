...
Provider state and SQLite data use separate named volumes. The default Compose stack does not mount the Docker socket. The runner override adds `/var/run/docker.sock` only for scenarios that actually launch Docker; that socket grants host-level control and must be treated as a privileged opt-in. The browser remains published on host loopback even though Express binds to all container interfaces internally.
