...
import ImplementationReview from './common/components/ImplementationReview/ImplementationReview';
import './styles.css';
...
function RunCard({ run, now }: { run: RunRecord; now: number }) {
  ...
  return <article className="run">
    ...
    <dl className="run-details"><div><dt>Agent</dt><dd>{run.provider ?? 'codex'} / {run.model.replace('gpt-5.6-', '')}</dd></div><div><dt>Reasoning</dt><dd>{run.reasoningEffort}</dd></div><div><dt>Feature type</dt><dd>{run.featureType ?? 'full-stack'}</dd></div>{duration !== null && <div><dt>{run.status === 'running' ? 'Elapsed' : 'Duration'}</dt><dd>{formatDuration(duration)}</dd></div>}<div className="artifact-detail"><dt>Local artifacts</dt><dd title={run.artifactPath}>{run.artifactPath}</dd></div></dl>
    {result && <><p className="score">{result.medianScore ?? '—'}%</p><p className="misses">{misses.length ? `Missed: ${misses.join(', ')}` : 'All structural contracts found.'}</p>{result.implementationReview?.length ? <ImplementationReview sections={result.implementationReview}/> : null}</>}
...
export default function App() {
  ...
  async function connect(repo = input.repo) {
    ...
    try {
      ...
      setMessage(`Repository ready. AGENTS.md and ${result.skills.length} skill${result.skills.length === 1 ? '' : 's'} discovered.`);
    } catch (error) { setMessage((error as Error).message); }
  }
      ...
      <div className="layout">
        <section className="panel" aria-labelledby="configure-title"><div className="panel-heading"><div><span>NEW RUN</span><h2 id="configure-title">Configure agent run</h2></div><span className="step">01</span></div>
          <form onSubmit={submit}>
            ...
            <fieldset><legend>Feature scope</legend><label htmlFor="feature-type">What kind of feature is this?</label><FloatingSelect id="feature-type" value={input.featureType} options={[{ value: 'full-stack', label: 'Full stack' }, { value: 'frontend', label: 'Frontend' }, { value: 'backend', label: 'Backend' }]} onChange={value => setInput({ ...input, featureType: value as StartRunInput['featureType'] })}/><p className="help">The agent follows AGENTS.md and selects the applicable repository skill itself.</p></fieldset>
