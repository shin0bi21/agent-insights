...
The primary live panel presents current context pressure, active worker and turn progress, and a bounded recent-prompt timeline. Stacked token movement and context-pressure charts visualize the recent series, while an accessible detailed-value table covers the same prompt window. Change-backed prompts are marked and expand inline to show their persisted preparation, discovery, execution, and classification evidence; there is no separate directive-episode panel. Each privacy-safe prompt interval derives cached input, new input, output, context at the opening boundary, and elapsed boundary time from monotonic root-agent counters. Counter resets or incomplete imported telemetry are unavailable rather than zero. Prompt text and private reasoning are never returned. Model and reasoning usage remains a lower drill-down: expanding the Main Agent or Subagents total separates each model-and-reasoning aggregate into cached input, new input, and output without exposing individual worker identities.
...
The product has two measurement modes: Session Review for real sessions and Benchmark Lab for controlled sandbox comparisons. Session Review's watcher defaults to sessions observed within five minutes and can widen to the last hour or 24 hours; older saved/importable history remains in its review area. Every recent result shows its relative activity time. The prompt timeline is root-agent only: its elapsed value measures one prompt boundary to the next, or to the latest observation for active work, rather than exact model execution time. Subagent tokens cannot yet be attributed to a root prompt. Usage is also presented as one Main agent total and one Subagents total; each can expand into its model-and-reasoning breakdown instead of exposing every subagent row by default. Worker date ranges show each matching group's current cumulative processed-token total. Live polling is not yet connected to the durable batching layer event-by-event. Historical imports do not currently recover complete command/check details, timestamps for every item, or file-level changes. Static reviews created before prompt telemetry was introduced show timeline evidence as unavailable until regenerated from a local rollout. Directive classification does not establish semantic adherence, related-topic linkage, or causation between context/delegation and outcome. Codex's local schema and generated app-server schema are internal compatibility boundaries, so adapters validate expected tables and paths and fail closed when they change. Session deletion, direct control-socket streaming, subagent-to-directive attribution, semantic directive evaluators, periodic subagent usage history, and versioned session scores remain follow-up features.
